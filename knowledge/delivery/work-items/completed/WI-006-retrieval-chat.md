...
>
> 2026-09-08: **Drift closed (commit `71a6583`)** — the completed code had kept the Aurora RDS Data API path (`retrieveChunks` via `RDSDataClient`/`ExecuteStatementCommand`) even after ADR-008; the WI-004 merge rewired ingest but not retrieval. `retrieveChunks` now takes an injected `NeonQueryFn` (positional `$1..$4` SQL, `embedding <=> $1::vector`), `ChatRouteEnv` carries `neonParameterName` (SSM SecureString / `NEON_DATABASE_URL` sandbox shortcut) instead of the `rds{}` block, and the R-1 tenant-scope contract is preserved and re-tested (chatbot-row values only). Full functions suite green; live sandbox chat returns real Neon pgvector chunks.
